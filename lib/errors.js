...
Errors.prototype = {
    ...
    explainError: function(error, colorize) {
        ...
        result.unshift(formatErrorMessage(error.message, this.getFilename(), colorize, error));
        return result.join('\n');
    },
 ...
 * @returns {String}
 */
function formatErrorMessage(message, filename, colorize, error) {
...
Errors.getPosition = function(element, offset) {
    ...
    if (offset === undefined) {
        if (element.sourceCodeLength === 1) {
            ...
            offset = (element.newlineCount === 0 && Math.ceil(element.sourceCodeLength / 2)) || 0;
        }
    }
    ...
    var newlineCount = element.newlineCount;
    if (newlineCount > 0) {
        var code = element.sourceCode;
