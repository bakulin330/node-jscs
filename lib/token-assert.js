...
TokenAssert.prototype.spacesBetween = function(options) {
    ...
    var fixed = !options.token.nextNonWhitespaceToken.isComment;
...
TokenAssert.prototype.linesBetween = function(options) {
    ...
    var normalizeToken = function(val){
        switch (val){
            case '\n': val = '[LF]'; break;
            case '\r': val = '[CR]'; break;
            case '\t': val = '[tab]'; break;
    ...
    var emitError = function(countPrefix, lineCount) {
        var msgPrefix = '"' + normalizeToken(token.value) + '" and "' + normalizeToken(nextToken.value) + '"';
